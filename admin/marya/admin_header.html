...
<link rel="stylesheet" type="text/css" media="screen" href="{STYLE_PATH_ADMIN}css/bootstrap-theme.min.css" />
...
<body>
    ...
    <div class="navbar navbar-default navbar-static-top navbar-inverse">
      <div class="container">
        <div class="navbar-header <IF NAME="{lang.DIR} == rtl">navbar-right</IF>">
          <button type="button" class="navbar-toggle" data-toggle="collapse" data-target=".navbar-collapse">
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
            <span class="icon-bar"></span>
          </button>
          <a class="navbar-brand" href="./"><img src="{STYLE_PATH_ADMIN}images/logo.png" style="height:25px" title="kleeja"> <span class="label label-default">{assigned_klj_ver}</span></a>
        </div>
        <div class="navbar-collapse collapse">
          <ul class="nav navbar-nav <IF NAME="{lang.DIR} == ltr">navbar-right</IF>">
 ...
 <div class="container">
	<!--  menu -->
   <div class="col-xs-6 col-sm-3 sidebar-offcanvas <IF NAME="{lang.DIR} == rtl">pull-right</IF>" id="sidebar" role="navigation">
      <div class="well sidebar-nav">
        <ul class="nav nav-pills nav-stacked">
			<li class="({go_to}==start?active:)">
				<a title="{lang.R_CPINDEX}" class="cpindex" href="./">{lang.R_CPINDEX}</a>
			</li>
	
			<LOOP NAME="adm_extensions_menu">
			<li class="({{current}}?active:)">
				<a title="{{title}}" class="side_anchor" href="{{link}}#!cp={{goto}}" <IF LOOP="confirm">onclick="javascript:confirm_from();"</IF>>
					{{title}} {{kbubble}}
				</a>
			</li>
			</LOOP>
...
<IF NAME="{go_to}==a_configs"><div class="panel panel-default"><div class="panel-body" style="font-size:12px;"></IF>
  <ul class="nav nav-({go_to}==a_configs?pills:tabs)">
	  {go_menu_html}
  </ul>
<IF NAME="{go_to}==a_configs"></div></div></IF>
</IF>
